...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initNavigation() {
        ...
        // 判断是否在子目录中
        const isInSubdir = currentPath.includes('/guide/') && !currentPath.endsWith('/guide/index.html') && !currentPath.endsWith('/guide/');
        ...
        menuItems.forEach(item => {
            ...
            let isActive = false;
            if (item.id === 'home') {
                // 首页：只有在 /guide/ 或 /guide/index.html 时才高亮
                isActive = currentPath.endsWith('/guide/') || currentPath.endsWith('/guide/index.html');
            } else {
                // 其他页面：精确匹配路径
